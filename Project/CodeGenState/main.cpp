...
int main()
{
	...
	wstring strCppPath = strProjPath + L"\\States\\qStateMgr.cpp";
	wstring strHeaderPath = strProjPath + L"\\States\\qStateMgr.h";
	...
	fwprintf_s(pFile, L"class qState;\n\n");

	fwprintf_s(pFile, L"class qStateMgr\n{\n");
	fwprintf_s(pFile, L"public:\n\tstatic void GetStateInfo(vector<wstring>& _vec);\n");
	fwprintf_s(pFile, L"\tstatic qState * GetState(const wstring& _strStateName);\n");
	fwprintf_s(pFile, L"\tstatic qState * GetState(UINT _iStateType);\n");
	fwprintf_s(pFile, L"\tstatic const wchar_t * GetStateName(qState * _pState);\n};\n");
	...
	fwprintf_s(pFile, L"#include \"qStateMgr.h\"\n\n");
	...
	// 두번째 함수 작성
	fwprintf_s(pFile, L"qState * qStateMgr::GetState(const wstring& _strStateName)\n{\n");
	...
	// 세번째 함수
	fwprintf_s(pFile, L"qState * qStateMgr::GetState(UINT _iStateType)\n{\n");
	...
	// 네번째 함수
	fwprintf_s(pFile, L"const wchar_t * qStateMgr::GetStateName(qState * _pState)\n{\n");
