...
void qFBXLoader::LoadFbx(const wstring& _strPath)
{
...
void qFBXLoader::LoadMeshDataFromNode(FbxNode* _pNode)
{
	// 노드의 메쉬정보 읽기
...
void qFBXLoader::LoadMesh(FbxMesh* _pFbxMesh)
{
...
void qFBXLoader::LoadMaterial(FbxSurfaceMaterial* _pMtrlSur)
{
...
void qFBXLoader::GetTangent(FbxMesh* _pMesh
...
void qFBXLoader::GetBinormal(FbxMesh* _pMesh, tContainer* _pContainer, int _iIdx, int _iVtxOrder)
{
...
void qFBXLoader::GetNormal(FbxMesh* _pMesh, tContainer* _pContainer, int _iIdx, int _iVtxOrder)
{
...
void qFBXLoader::GetUV(FbxMesh* _pMesh, tContainer* _pContainer, int _iIdx, int _iUVIndex)
{
...
Vec4 qFBXLoader::GetMtrlData(FbxSurfaceMaterial* _pSurface, const char* _pMtrlName, const char* _pMtrlFactorName)
{
	FbxDouble3  vMtrl;
	FbxDouble	dFactor = 0.;
...
wstring qFBXLoader::GetMtrlTextureName(FbxSurfaceMaterial* _pSurface, const char* _pMtrlProperty)
{
	string strName;
...
void qFBXLoader::LoadTexture()
{
	path path_content = qPathMgr::GetInst()->GetContentPath();
	...
	for (UINT i = 0; i < m_vecContainer.size(); ++i)
	{
		for (UINT j = 0; j < m_vecContainer[i].vecMtrl.size(); ++j)
		{
			...
			for (size_t k = 0; k < vecPath.size(); ++k)
			{
				...
				path_dest = qPathMgr::GetInst()->GetRelativePath(path_dest);
				qAssetMgr::GetInst()->Load<qTexture>(path_dest, path_dest);
...
void qFBXLoader::CreateMaterial()
{
	wstring strMtrlName;
	wstring strPath;
	...
	for (UINT i = 0; i < m_vecContainer.size(); ++i)
	{
		for (UINT j = 0; j < m_vecContainer[i].vecMtrl.size(); ++j)
		{
			...
			Ptr<qMaterial> pMaterial = qAssetMgr::GetInst()->FindAsset<qMaterial>(strPath);
			...
			pMaterial->SetShader(qAssetMgr::GetInst()->FindAsset<qGraphicShader>(L"Std3D_DeferredShader"));

			wstring strTexKey = m_vecContainer[i].vecMtrl[j].strDiff;
			Ptr<qTexture> pTex = qAssetMgr::GetInst()->FindAsset<qTexture>(strTexKey);
			if (NULL != pTex)
				pMaterial->SetTexParam(TEX_PARAM::TEX_0, pTex);

			strTexKey = m_vecContainer[i].vecMtrl[j].strNormal;
			pTex = qAssetMgr::GetInst()->FindAsset<qTexture>(strTexKey);
			if (NULL != pTex)
			...
			qAssetMgr::GetInst()->AddAsset<qMaterial>(pMaterial->GetKey(), pMaterial.Get());
			pMaterial->Save(qPathMgr::GetInst()->GetContentPath() + strPath);
		}
	}
}

void qFBXLoader::LoadSkeleton(FbxNode* _pNode)
{
...
void qFBXLoader::LoadSkeleton_Re(FbxNode* _pNode, int _iDepth, int _iIdx, int _iParentIdx)
{
	FbxNodeAttribute* pAttr = _pNode->GetNodeAttribute();
...
void qFBXLoader::LoadAnimationClip()
{
...
void qFBXLoader::Triangulate(FbxNode* _pNode)
{
	FbxNodeAttribute* pAttr = _pNode->GetNodeAttribute();
...
void qFBXLoader::LoadAnimationData(FbxMesh* _pMesh, tContainer* _pContainer)
{
...
void qFBXLoader::LoadWeightsAndIndices(FbxCluster* _pCluster, int _iBoneIdx, tContainer* _pContainer)
{
...
void qFBXLoader::LoadOffsetMatrix(FbxCluster* _pCluster, const FbxAMatrix& _matNodeTransform, int _iBoneIdx, tContainer* _pContainer)
{
...
void qFBXLoader::LoadKeyframeTransform(FbxNode* _pNode, FbxCluster* _pCluster, const FbxAMatrix& _matNodeTransform, int _iBoneIdx, tContainer* _pContainer)
{
...
int qFBXLoader::FindBoneIndex(string _strBoneName)
{
...
FbxAMatrix qFBXLoader::GetTransform(FbxNode* _pNode)
{
...
void qFBXLoader::CheckWeightAndIndices(FbxMesh* _pMesh, tContainer* _pContainer)
{
